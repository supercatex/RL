...
class DeepQLearning(object):
    ...
    def build_model(self) -> models.Model:
        ...
        x = x_in
        if self.with_cnn:
            x = layers.Conv2D(32, 3, 1, "same", activation=activations.relu)(x)
    ...
    def update_target_network(self, tau: float = 1.0):
        qw = self.q_model.get_weights()
        tw = self.t_model.get_weights()
        nw = []
        for i in range(len(qw)):
            nw.append(tw[i] * (1.0 - tau) + qw[i] * tau)
        self.t_model.set_weights(nw)
